...
export async function proxy(request: NextRequest) {
    ...
    // FIX 1: Use JWT_REFRESH_SECRET for the refresh token, NOT JWT_ACCESS_SECRET
    const decodedRefreshToken = refreshToken ? jwtUtils.verifyToken(refreshToken, process.env.JWT_REFRESH_SECRET as string) : null;

    // Check if access token expired but refresh token is still valid
    ...
    // FIX 2: Handle users without a valid access token gracefully
    if (!decodedAccessToken?.success) {
        cookieStore.delete("accessToken"); // Clear invalid token
        ...
        // If it's a public or auth route, let them pass without a token
        return NextResponse.next();
    }
    ...
    // Authorization: Role-based protection for dashboard routes
    if (pathname.startsWith("/dashboard") && userRole !== "USER") {
        return NextResponse.redirect(new URL('/not-found', request.url));
    }
    if (pathname.startsWith("/admin-dashboard") && userRole !== "ADMIN") {
